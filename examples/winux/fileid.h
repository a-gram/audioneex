...
class IdentificationTask : public IdTask
{
    ...
    Identify(const std::string &audioclip)
    {
        ...
        {
           auto filename = bfs::path(audioclip).filename().string();
           ...
           ibuffer.Create(size_t(11025 * 1.2), 11025, 1);
...
public:
    ...
    void 
    Connect(IdResultsListener *listener)
    {
        m_Listener = listener;
    }
...
class FileIdResultsParser : public IdResultsListener
{
    std::shared_ptr<KVDataStore>
    m_Datastore;
    
    std::shared_ptr<Audioneex::Recognizer>
    m_Recognizer;

  public:

    void 
    OnResults(const Audioneex::IdMatch *results)
    {
        ...
        if(BestMatch.size() == 1)
        {
           ...
           if(BestMatch[0].IdClass == Audioneex::IDENTIFIED)
           {
               ...
               std::cout << "Id.Time: " << m_Recognizer->GetIdentificationTime() 
                         << "s" << std::endl;
           ...
           {
               std::cout << "FATAL ERROR: Unexpected identification class" 
                         << std::endl;
           }
        ...
        std::cout << "ID Time: " 
                  << m_Recognizer->GetIdentificationTime() << " s" 
                  << std::endl;
    }

    void 
    SetDatastore(std::shared_ptr<KVDataStore> &store)
    {
        m_Datastore = store;
    }
