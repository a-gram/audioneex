...
/// LF comparison operator
inline bool 
operator == (const LocalFingerprint_t &lf1,
...
class AUDIONEEX_API_TEST Fingerprinter
{
    static const int 
    POI_LOCATION = -1;

    /// Object providing common audio signal processing
    AudioProcessor<int16_t>
    m_AudioProcessor;
    
    /// The overlap&shift buffer for the STFT
    AudioBuffer<float>
    m_OSBuffer;
    
    /// The overlap&shift window for the STFT
    AudioBuffer<float>
    m_OSWindow;
    ...
    /// Instantaneous FFT frame
    std::vector<float> 
    m_fftFrame;
    
    /// The list of local fingerprints for the current audio
    lf_vector
    m_LF;
    
    /// LF identifier counter
    int
    m_LID;
    ...
    std::vector<std::vector<float> > 
    m_POI;  // For display purposes only
    
#endif

    /// Compute the spectrum of the given audio buffer
    void 
    ComputeSpectrum(AudioBuffer<float> &audio, bool flush);
    
    /// Peak finding algorithm
    void 
    FindPeaks();
    
    /// POI extraction algorithm
    void 
    ExtractPOI();
    
    /// Compute the local fingerprints
    void 
    ComputeDescriptors();
    
    /// Compute the energy of the scanning window
    float 
    ComputeWindowEnergy(int WoT, int WoF, 
                        int rWT, int rWF,
                        std::vector< std::vector<float> > &X);
    
    /// Compute the mean energy of the scanning window
    float 
    ComputeMeanWindowEnergy(int WoT, int WoF, 
                            int rWT, int rWF,
                            std::vector< std::vector<float> > &X);
 ...
 public:
    ...
    Fingerprinter(size_t bufferSize = Pms::Fs * 2 + Pms::OrigWindowSize);
   ...
   ~Fingerprinter() = default;
    ...
    void 
    Process(AudioBuffer<float> &audio, bool flush=false);
