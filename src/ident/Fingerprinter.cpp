...
Audioneex::Fingerprinter::Fingerprinter(size_t bufferSize)
:
...
void 
Audioneex::Fingerprinter::Process(AudioBuffer<float> &audio, bool flush)
{
    ...
    if(audio.Duration() >= 0.5)
    {
       ...
       if( m_OSBuffer.Capacity() < audio.Size() + Pms::OrigWindowSize )
       {
          ...
          m_OSBuffer = AudioBuffer<float>(audio.Size() + Pms::OrigWindowSize,
                                          Pms::Fs,
                                          Pms::Ca,
                                          0);
       }
...
void 
Audioneex::Fingerprinter::SetBufferSize(size_t size)
{
    m_OSBuffer = AudioBuffer<float>(size + Pms::OrigWindowSize, 
                                    Pms::Fs, 
                                    Pms::Ca, 
                                    0);
}
...
void 
Audioneex::Fingerprinter::ComputeSpectrum(AudioBuffer<float> &audio, bool flush)
{
    ...
    for(size_t wstart = 0; 
        m_OSWindow.Size() == Pms::OrigWindowSize; 
        wstart += Pms::hopSize)
    {
        m_OSBuffer.GetSlice(wstart, Pms::OrigWindowSize, m_OSWindow);

        // if we have a complete FFT window, process it
        if(m_OSWindow.Size() == Pms::OrigWindowSize)
        {
           m_AudioProcessor.FFT_Transform(m_OSWindow, 
                                          m_fftFrame, 
                                          FFT::EnergySpectrum);
...
void 
Audioneex::Fingerprinter::FindPeaks()
{
...
void 
Audioneex::Fingerprinter::ExtractPOI()
{
...
void 
Audioneex::Fingerprinter::ComputeDescriptors()
{
...
float 
Audioneex::Fingerprinter::ComputeWindowEnergy(int WoT, int WoF, 
                                              int rWT, int rWF,
                                              std::vector< std::vector<float> > &X)
{
    ...
    for(int u = WoT - rWT; u <= WoT + rWT; u++)
    {
        ...
        {
            // check that point is within spectrum bounds
            if(u >= 0 && u < X.size() && v >= 0 && v < X[0].size())
               EW += std::abs( X[u][v] );
        }
    }
    return EW;
...
float 
Audioneex::Fingerprinter::ComputeMeanWindowEnergy(int WoT, int WoF, 
                                                  int rWT, int rWF,
                                                  std::vector< std::vector<float> > &X)
{
    float EW = 0.0f;
