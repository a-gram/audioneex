...
#define ENGINE_VERSION      010400L
#define ENGINE_VERSION_STR  "1.4.0"
...
/// by the recognizer to perform the identification. They may be used to
/// set the trade-off between fast response times and accuracy.
enum eIdentificationType
{
     /// Perform identification using a 3-class fuzzy 
     /// classifier (see Audioneex::eIdClass). Provides 
     /// more robustness to noise by reducing false positives 
     /// but takes more time to give a response as it requires 
     /// more audio to find a match.
...
/// These labels are attached to the results returned by Recognizer::GetResults()
/// to indicate the outcome of the identification.
...
struct IdMatch
{
    ...
    ///       happen that there are parts in a recording that are perceptually 
...
inline bool 
IsNull(const PListBlockHeader& hdr)
{
    return hdr.ID == 0 && 
           hdr.BodySize == 0 && 
           hdr.FIDmax == 0;
...
/// these structures. Note that how these structures are organised in the data 
/// store is irrelevant to the identification engine, as long as they are returned 
...
public:
    ...
    ///                   @note If the list or block does not exist the method shall 
    ///                   return a null header (a zero-initialized header).
    virtual PListBlockHeader 
    OnIndexerBlockHeader(int lid, int bid) = 0;

    /// This handler is called by the indexer whenever a new block chunk is produced.
    /// When the indexer's cache is flushed, its contents are processed and block 
    /// chunks emitted for the data store to process. Logically, these chunks are 
    /// appended to the specified list as part of the specified block (the last 
    /// block, also referred to as the "append block"), although the data store 
    /// implementation is free to organize the data as it likes. However, remember 
    /// that when blocks are requested by the engine, they have to be returned 
    /// exactly as they had been emitted. See [link] for more details. Also, since 
    /// the list (and the block) to which we're appending are being modified,
    /// the updated headers are emitted as well. Clients should use them to update 
    ...
    /// @param[in]  hdr    The header of the block to which to append the chunk. 
    ///                    This is actually always the last block in the list 
    ///                    (the "append block").
    ...
    /// @param[in]  chunk_size  The size in bytes of the chunk's data.
    virtual void 
    OnIndexerNewBlock(int lid,
                      PListHeader &lhdr,
                      PListBlockHeader &hdr,
                      uint8_t* chunk, 
                      size_t chunk_size) = 0;

    /// As part of the indexing process, a fingerprint for each processed audio 
    /// recording is emitted. This method is called by the indexer to signal the 
    /// creation of the fingerprint. Clients may choose not to implement this method 
    /// if the original fingerprints are not needed (for example if MMS is always 
    /// set to zero the only match level used by the algorithm will not use the 
    /// fingerprints original data).
    ///
    /// @param[in] FID        The fingerprint's unique identifier.
    /// @param[in] data       Pointer to the memory location containing the 
    ///                       fingerprint's data.
    ...
    /// This method is called by the engine during the identification stage. It 
    /// shall return the list's block for the specified list id from the 
    /// fingerprints index. Although clients are free to implement their storage 
    /// solutions and layouts, index blocks, along with their headers, must be 
    /// returned as they have been emitted during the indexing stage. So, if the 
    /// datastore implementation applies some sort of transformation to the blocks 
    //  data, the inverse must be applied prior to returning the block's data to 
    /// the engine.
    ///
    /// @param[in]  lid        The identifier of the list from which to retrieve 
    ///                        the block.
    ...
    ///                        in the returned data. If true the block's header 
    ///                        must be returned prepended to the block's body.
    /// @return                A pointer to the memory location containing the 
    ///                        block's data. Clients must ensure that the returned 
    ///                        pointer remains valid after the method returns, 
    ///                        for example by storing the data into a session-bound 
    ///                        buffer. See [link] for more details.
    ///                        @note A null pointer and zero size shall be returned 
    ///                        if the block is not found.
    virtual const uint8_t* 
    GetPListBlock(int lid, 
                  int bid,
                  size_t& data_size, 
    ...
    virtual size_t 
    GetFingerprintSize(uint32_t FID) = 0;

	/// Get fingerprint data from the datastore. The match algorithm does not need 
    /// the entire fingerprints for recognition but only small chunks of them, so 
    /// this method is flexible with respect to the amount of data that can be 
    /// retrieved, being it the whole fingerprint or just portions of it. This 
    /// method is never called if the MMS parameter is set to 0 because the 
    /// fingerprints data is not needed in such a case.
    ///
    /// @param[in]  FID    The fingerprint's unique identifier.
    /// @param[out] read   The size of the data actually read.
    /// @param[in]  nbytes The size in bytes of the data to be read. If it's set 
    ///                    to zero, the whole fingerprint starting at @p bo shall 
    ///                    be returned.
    /// @param[in]  bo     The offset in bytes within the fingerprint at which to 
    ///                    start reading.
    /// @return            A pointer to the memory location containing the 
    ///                    fingerprint's data. The engine does not take ownership 
    ///                    of this pointer, which must remain valid after the method 
    ///                    returns. This can be achieved by storing the data into a 
    ///                    session-bound buffer (see [link] for more details).
    virtual const uint8_t* 
    ...
    /// Check whether the data store is open (ready for operations)
    ///
    /// @return            A bool value indicating the open status.
    virtual bool 
    IsOpen() const = 0;
...
public:
    ...
    ///                      The audio must be 16 bit, mono, 11025Hz normalized 
    ///                      in [-1,1].
    /// @param[in]  nsamples The number of requested samples.
    /// @return              The number of samples actually read or a negative 
    ///                      value on error. When all the audio for a recording 
    ///                      is consumed the audio provider must signal this to 
    ///                      the engine by returing 0.
    virtual int 
    OnAudioData(uint32_t FID, 
                float* buffer, 
                size_t nsamples) = 0;
...
public:
    ...
    ///          the fingerprints.
    virtual void 
    SetMatchType(eMatchType type) = 0;
    ...
    virtual void 
    SetMMS(float value) = 0;

    /// Set the type of identification to be used.
    ///
    /// @param[in]  type  The identification type. This must be one of the 
    ///                   supported values in Audioneex::eIdentificationType.
    virtual void 
    SetIdentificationType(eIdentificationType type) = 0;

    /// Set the classification mode used by the fuzzy classifier.
    ///
    /// @param[in]  mode  The identification mode. This must be one of the 
    ///                   supported values in Audioneex::eIdentificationMode.
    virtual void 
    SetIdentificationMode(eIdentificationMode mode) = 0;
    ...
    virtual void 
    SetBinaryIdThreshold(float value) = 0;
	...
	virtual void 
    SetBinaryIdMinTime(float value) = 0;
    ...
    virtual const IdMatch* 
    Identify(const float *audio, size_t nsamples) = 0;

    /// Call this method to check the current state of the identification.
    /// Usually this is done right after calling Identify().
    ///
    /// @return  A pointer to an array of Audioneex::IdMatch structures representing 
    /// the best match(es). Usually there would be only one match, although ties 
    /// may occur. The end of the array is marked by a 'null' element, which is 
    /// an IdMatch structure set to zero (you can use Audioneex::IsNull(IdMatch) 
    /// to check for that), so the number of elements in the array is always greater 
    /// than zero. If no identification could be made, the results set will contain 
    /// only the null element. The recognizer will always return a results object 
    /// after a set period of time has elapsed, which can be empty if no match was 
    /// found or an array with the best match(es). If the identification cannot be 
    /// completed (e.g. for insufficient audio) the returned pointer will be null, 
    /// so clients should always check its validity.
    ///
    /// @note The returned pointer is owned by the identification engine and must 
    ///       not be deleted nor retained by clients.
    virtual const IdMatch* 
    GetResults() = 0;
    ...
    virtual void 
    SetDataStore(DataStore* dstore) = 0;
...
/// structures for storage. Use this interface to build the "fingerprints database".
...
public:
    ...
    /// Set the working cache size (in MB). The working cache is a temporary
    /// memory area used by the indexer when building the fingerprints database.
    /// All the data structures are incrementally built in the cache and then 
    /// 'flushed' to disk once the cache limit is reached. Smaller values will
    /// cause frequent disk writes, while bigger values will cause sporadic 
    /// bursts of disk writes.
    ///
    /// @param[in]  limit  The memory limit in MB.
    virtual void 
    SetCacheLimit(size_t limit) = 0;

    /// Get the currently set cache limit.
    ///
    /// @return The currently set limit in MB
    virtual size_t 
    GetCacheLimit() const = 0;

    /// Get the amount of memory currently used by the cache.
    ///
    /// @return  The current cache size in bytes.
    virtual size_t 
    GetCacheUsed() const = 0;
    ...
    virtual void 
    SetAudioProvider(AudioProvider* aprovider)= 0;

    /// Get the currently set audio provider.
    virtual AudioProvider* 
    GetAudioProvider() const = 0;
...
/// This exception is raised when invalid operations on the data store are
/// performed.
class AUDIONEEX_API DatastoreException : public Audioneex::Exception 
{
  /// Constructor
  public: explicit DatastoreException(const std::string& msg) :
                   Audioneex::Exception(msg) {}
};
...
/// Interface for user-level access to the engine's data store

/// This interface extends the DataStore interface by adding common database 
/// operations. Users of the engine will use this interface to access the
/// fingerprints database at the application level. It provides simple get/put 
/// operations typical of key-value data stores.

class AUDIONEEX_API KVDataStore : public DataStore
{
public:
    ...
    enum eOperation
    {
        /// Indicates that the database is used only to fetch data (read mode)
        FETCH,
        
        /// Indicates that the database is being built (write mode)
        BUILD,
        
        /// Indicates that the database is in build&merge mode (read/write mode)
        BUILD_MERGE
    };
    ...
    virtual size_t 
    GetFingerprintsCount() = 0;

    /// Save a fingerprint in the datastore
    virtual void 
    PutFingerprint(uint32_t FID, 
                   const uint8_t* data, 
                   size_t size) = 0;

    /// Write metadata associated to a fingerprint
    virtual void 
    PutMetadata(uint32_t FID, const std::string& meta) = 0;

    /// Get metadata associated to a fingerprint
    virtual std::string 
    GetMetadata(uint32_t FID) = 0;

    /// Save datastore info
    virtual void 
    PutInfo(const std::string& info) = 0;
